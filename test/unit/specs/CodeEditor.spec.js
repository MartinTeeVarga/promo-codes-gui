...
describe('CodeEditor.vue', () => {
  describe('REST calls', () => {
    ...
    function createAndRouteToCodeEditor (queryParams) {
      console.log('Query used: ' + JSON.stringify(queryParams))
      router.push({path: 'code', query: queryParams})
    }

    it('should render empty form on new', done => {
      createAndRouteToCodeEditor()
      Vue.nextTick(() => {
    ...
    it('should render filled form on edit', done => {
      ...
      createAndRouteToCodeEditor({gameId: 'GM01', codeId: 'CD01'})
      Vue.nextTick(() => {
        moxios.wait(() => {
